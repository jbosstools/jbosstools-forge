...
public abstract class QuickAccessContents {
	...
	private int computeNumberOfItems() {
		...
		return (rect.height - headerHeight + itemHeight - 1)
				/ (itemHeight + table.getGridLineWidth());
	}
	...
	public void refresh(String filter) {
		...
		if (table.getItemCount() > 0) {
		...
		} else if (filterTextEmpty) {
			{
				...
				item.setText(0, "Available Categories:");
	 ...
	 * @return
	 */
	public abstract QuickAccessElement getPerfectMatch(String filter);

	/**
	 * @param filterTextEmpty
	 */
	public abstract void updateFeedback(boolean filterTextEmpty,
		...
		for (int i = 0; i < providers.length; i++) {
			if (entries[i] != null) {
				...
				for (Iterator it = entries[i].iterator(); it.hasNext();) {
					...
					if (Util.isWpf()) {
						item.setImage(1,
								entry.getImage(entry.element, resourceManager));
					}
					index++;
				}
			}
	...
	private List[] computeMatchingEntries(String filter,
		...
		do {
			...
			for (int i = 0; i < providers.length
				...
				int count = 0;
				QuickAccessProvider provider = providers[i];
				if (filter.length() > 0 || provider.isAlwaysPresent()
						|| showAllMatches) {
					List<QuickAccessElement> elements = provider
							.getElementsSorted();
					int j = indexPerProvider[i];
					while (j < elements.size()
							&& (showAllMatches || (count < countPerProvider && countTotal < maxCount))) {
						QuickAccessElement element = elements.get(j);
		...
		if (!perfectMatchAdded) {
			QuickAccessEntry entry = perfectMatch == null ? null : perfectMatch
					.match(filter, providers[0]);
			if (entry != null) {
				if (entries[0] == null) {
	...
	public abstract void handleElementSelected(String text,
