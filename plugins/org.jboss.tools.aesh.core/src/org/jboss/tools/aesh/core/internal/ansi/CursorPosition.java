...
public class CursorPosition extends AbstractCommand {
	...
	// need to subtract 1 from the arguments as the ansi cursor position
	// arguments are 1-based
	public CursorPosition(String arguments) {
		int i = arguments.indexOf(';');
		if (i != -1) {
			line = Integer.parseInt(arguments.substring(0, i)) - 1;
			column = Integer.parseInt(arguments.substring(i + 1)) - 1;
		} else if (arguments.length() > 0) {
			line = Integer.parseInt(arguments) - 1;
		}
	}
